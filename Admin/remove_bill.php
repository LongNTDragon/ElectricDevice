...
<?php
    ...
    {
        $bD = new BillDetail();
        $bD->billID = $billArr[0]->billID;
        $bDArr = $bD->getAllProductByID($con);

        foreach($bDArr as $row)
        {
            $bD->proID = $row->proID;
            $bD->deleteProduct($con);
        }
...
<div class="text-white text-center container-fluid p-2 pb-3" style="margin-top: 5rem; margin-bottom:3.8rem;
    ...
    <h3 class="pt-4">Bạn có chắc là muốn xóa đơn hàng <b class="text-info"><?= $billArr[0]->billID?></b> ?</h6>
